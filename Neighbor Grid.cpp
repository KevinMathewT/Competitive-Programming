...
void solve(){
	...
	fi (0, n - 1) if (a[i][0] > 3 || a[i][m - 1] > 3) {
		cout << "NO\n";
		return;
	}
	fi (0, m - 1) if (a[0][i] > 3 || a[n - 1][i] > 3) {
		cout << "NO\n";
		return;
	}
	fi (0, n - 1) fj (0, m - 1) if (a[i][j] > 4) {
		cout << "NO\n";
		return;
	}
	...
	a[n - 1][0] = 2;
	a[n - 1][m - 1] = 2;
	fi (1, n - 2) a[i][0] = 3, a[i][m - 1] = 3;
	fi (1, m - 2) a[0][i] = 3, a[n - 1][i] = 3;
	fi (1, n - 2) fj (1, m - 2) a[i][j] = 4;
