...
const ll N = 100010;
ll B, L, D, S[N], NB[N], T[N], M[N], SU[N], SCORE[N], SCORE1[N];
vector<vector<ll> > ID;
...
void solve(){
  mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());
    ...
    SCORE[i] = 0;
		ID[i] = vector<ll>(NB[i]);
		for(ll j=0;j<NB[i];j++) cin >> ID[i][j], SCORE[i]++, SCORE1[i] += S[ID[i][j]];
	}
  ...
  for(ll i=0;i<L;i++){
    pair<ll, ll> pll = {0, 0};
    pll.S = i;
    pll.F = (SCORE[i] * SCORE[i] * SCORE[i]) / (T[i] * T[i] * NB[i]);
    SC[i] = pll;
  }
...
int main(){
	freopen("inputb.txt", "r", stdin);		//Comment
