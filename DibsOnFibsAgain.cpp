...
void te(){
	...
	for(ll i=0;i<m;i++){
		contria[i] = mod(mod(mod(a[i]-mina+1000000007) * fib[n-1]) * m);
		contrib[i] = mod(mod(mod(b[i]-minb+1000000007) * fib[n]) * m);
		// cout << contria[i] << " " << contrib[i] << "\n";
	}

	ll ans = mod(mod(mina * fib[n-1]) + mod(minb * fib[n]));
	ll result = 0;

	for(ll i=0;i<m;i++)
		result = mod(result + mod(mod((m * ans) + (contria[i])) + contrib[i]));
